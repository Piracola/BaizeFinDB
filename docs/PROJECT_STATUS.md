...
| M5 A 股 5 分钟资金主线雷达 MVP | 进行中 | 已有静态 Web 终端工作台（含报告、日报/周报、评分展示）、Telegram Bot MVP、Windows 客户端 MVP（含报告、日报/周报、评分展示）、Celery 5 分钟采集后扫描调度入口、持仓/自选最小 API 和 Web 维护视图、quick/standard 报告 MVP、日报/周报汇总 API、1d/3d/5d/10d 综合评分、Telegram 折叠推送日志和 P0 推送后 standard report 自动生成；后续继续补评分校准和生产化部署。 |
