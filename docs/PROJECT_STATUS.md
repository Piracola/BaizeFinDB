...
Windows 客户端 MVP 已补充为本地桌面入口，可连接本地或 Linux 服务器 API，查看健康状态、运行状态、运维历史、运行就绪自检、Tushare 数据源状态和准入自检、雷达总览、生命周期分布、市场情绪摘要、个股回推证据、信号列表、持仓、自选、报告摘要、日报/周报汇总、单信号 1d/3d/5d/10d v2 综合评分明细，维护 Telegram chat 绑定/白名单，并打开现有 Web 面板；它仍只消费后端结果，不重新计算雷达等级、运行状态、数据源状态或评分，也不是完整安装包。Windows 首次使用 smoke check 已接入，可在打开 GUI 前验证 Tkinter import、核心健康、OPS readiness 和非变更只读端点，空首用数据只作为 warning，并支持脱敏有界 JSON 输出；OPS readiness 统计窗口默认 24 小时，可配置为 1 到 168 小时，便于区分最近健康状态和历史 Provider / 数据质量 warning；`run-client.ps1 -SmokeCheck` 已可在 GUI 启动前执行同一套检查，复用 ServerUrl/UserKey，支持 `-SmokeLookbackHours`、`-SmokeJsonOutput` 和 `-SmokeStrict` warning 阻断模式。Tushare 状态/自检视图只读取 `/providers/tushare/status` 和 `/providers/tushare/readiness`，不触发真实抓取。
