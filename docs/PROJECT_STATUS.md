...
Windows 客户端已增加手动模型草稿入口：GUI `生成模型草稿` 按钮读取当前 Signal ID，调用 `POST /radar/signals/{signal_id}/model-analysis-draft`，展示后端返回的 model/draft 状态、provider/model/fallback/audit 元数据、脱敏 advisory 字段、blocked count 和 boundary。客户端不自动调用模型草稿、不本地调用模型、不重算模型状态、雷达定级、生命周期、审查状态、评分或报告状态，也不展示 prompt、raw response、raw source、blocked phrase text、个人持仓成本/仓位或交易指令。
...
| M2 数据底座 | 已完成早期闭环 | AKShare 行情/行业/概念最小 Provider，采集日志、快照、质量检查、Provider 查询 API；Tushare `daily`、`stock_basic`、`anns_d` 和 `stock_company` 已支持手动抓取、失败记录、日志、快照查询和只读准入自检，`daily` 为后续雷达/复盘/报告提供基础日线行情，`anns_d` 重大风险公告可被后续雷达扫描映射为 risk P0；`anns_d` Beat 调度有默认关闭的显式开关，并已有离线/no-token 预调度校验，尚未作为默认调度。 |
...
- `infra/scripts/verify_tushare_daily.py --json-output <path>`、`infra/scripts/verify_tushare_stock_basic.py --json-output <path>`、`infra/scripts/verify_tushare_announcements.py --json-output <path>`、`infra/scripts/verify_tushare_stock_company.py --json-output <path>`：真实 token live verify 脱敏 evidence 输出；不写数据库，成功和失败都会保存状态、端点、查询参数、行数、质量状态、必需字段、缺失字段和去 URL/source/token/secret-like 字段的少量归一化样例或错误摘要，样例值会递归脱敏并截断超长文本。
...
- Windows 客户端源码运行、本地 PyInstaller 脚手架和 Inno Setup 安装包脚手架已保留；Linux 服务器不能直接交叉生成 Windows `.exe`，仓库已新增 GitHub Actions Windows 打包工作流，推送后由 `windows-latest` runner 产出 onedir 客户端 artifact 和 installer artifact；打包脚手架显式加入 `clients.windows.*` hidden imports，CI 会用 `BAIZEFINDB_PACKAGED_IMPORT_CHECK=1` 启动打包后的 exe 做导入校验，避免 artifact 缺少 `clients` 包。
...
- Tushare 当前已支持 `daily`、`stock_basic`、`anns_d` 和 `stock_company` 手动抓取；四条 live verify 脚本均支持脱敏 JSON evidence 输出；`daily` 日线行情已能写入 Provider 快照供后续雷达/报告消费，`anns_d` 中明显重大风险公告已能被雷达扫描映射为 risk P0；`anns_d` Beat 调度默认关闭，后续在 checklist、`verify_tushare_anns_d_preflight.py` 离线预调度校验、真实 token live evidence、字段和误报样例稳定后再显式启用。
