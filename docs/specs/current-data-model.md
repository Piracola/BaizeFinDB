...
- Tushare `daily`、`stock_basic`、`anns_d` 和 `stock_company` 当前会复用 `market_snapshots`、`provider_fetch_logs` 和 `data_quality_checks` 写入日线行情、证券主数据、公告快照、公司信息快照、抓取日志和质量记录。`daily` 快照使用 `source_time` 记录最大 `trade_date`，供后续雷达、复盘和报告消费；`anns_d` 快照中的明显重大风险公告标题可被后续雷达扫描映射为 risk P0；普通公告不会生成信号。公告 `url` 只允许留在内部 Provider 快照中，不能进入公开分享 payload。
