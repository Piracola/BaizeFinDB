...
- Tushare `stock_basic` 和 `anns_d` 当前会复用 `market_snapshots`、`provider_fetch_logs` 和 `data_quality_checks` 写入证券主数据、公告快照、抓取日志和质量记录；公司信息仍是 API 能力声明，尚不写表。公告 `url` 只允许留在内部 Provider 快照中，不能进入公开分享 payload。
