...
- `server_deploy_check.py --check-m5-smoke` 会只读验证健康、OPS、Provider、Radar、Telegram、`/radar/signals` 和有信号时的 `/radar/signals/{id}/analysis` JSON 契约；空信号列表只记录 warning，避免阻断新服务器首次部署。`server_delivery_acceptance.py` 会保留 helper evidence JSON 的 warning 状态，汇总报告可见但仍零退出；缺失、损坏或失败 evidence 会标为 fail。
