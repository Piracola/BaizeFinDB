...
- `server_deploy_check.py --check-m5-smoke` 会只读验证健康、OPS、Provider、Radar、Telegram、`/radar/signals` 和有信号时的 `/radar/signals/{id}/analysis` JSON 契约；空信号列表只记录 warning，避免阻断新服务器首次部署。`server_delivery_acceptance.py` 默认验收 localhost，也可用 `--base-url <url>` 验证非默认端口、反向代理或域名地址；它会保留 helper evidence JSON 的 warning 状态，汇总报告可见但仍零退出，缺失、损坏或失败 evidence 会标为 fail；生产切换前可加 `--fail-on-warning`，让 warning-only 验收仍写 `warn` 报告但返回非零退出码；需要把脱敏 OPS evidence 放进同一验收包时，加 `--include-ops-evidence`。
