...
- 未来接入 LLM-backed 多 agent 前，先运行 `uv run python infra/scripts/model_provider_readiness.py --json-output evidence/model-provider-readiness.json` 做只读模型 Provider 配置自检；默认 `MODEL_ANALYSIS_ENABLED=false` / `MODEL_PROVIDER=disabled` 为 OK，不调用模型、不验证 token、不读取数据库、不生成报告、不发送 Telegram、不输出 API key。显式启用时，`openai` provider 要求 `MODEL_PRIMARY_MODEL` 和 `OPENAI_API_KEY`，`custom` provider 要求 `MODEL_PRIMARY_MODEL`、`MODEL_API_BASE_URL` 和 `MODEL_API_KEY`。
