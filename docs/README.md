...
- Linux 部署骨架还包含可选 `baizefindb-alert-telegram.service` oneshot 示例；它不会默认启用 timer，使用前必须先在服务器本地创建服务账号可读的 `/etc/baizefindb/telegram-alert.env`，service 会在发送前先写 env check evidence，并确认 dedupe state 生效。
