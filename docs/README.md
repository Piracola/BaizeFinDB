...
- 未来接入 LLM-backed 多 agent 前，先运行 `uv run python infra/scripts/model_provider_readiness.py --json-output evidence/model-provider-readiness.json` 做只读模型 Provider 配置自检；也可用 `server_deploy_check.py --check-model-provider-readiness --model-provider-readiness-json-output <path>` 或 `server_delivery_acceptance.py --include-model-provider-readiness` 纳入部署验收；`--production-readiness` 默认包含该 evidence。默认 `MODEL_ANALYSIS_ENABLED=false` / `MODEL_PROVIDER=disabled` 为 OK，不调用模型、不验证 token、不读取数据库、不生成报告、不发送 Telegram、不输出 API key。显式启用时，`openai` provider 要求 `MODEL_PRIMARY_MODEL` 和 `OPENAI_API_KEY`，`custom` provider 要求 `MODEL_PRIMARY_MODEL`、`MODEL_API_BASE_URL` 和 `MODEL_API_KEY`。
- 模型真实调用入口先走 `app.ai.model_client`：默认关闭，启用后只提供 OpenAI-compatible `/chat/completions` 客户端、fallback/degraded 审计包装和测试注入 transport。该模块只在 fallback 或 degraded 时写 `model_call_logs`，主模型成功不写审计；当前不接入雷达分析 API，不修改 P0/P1/P2、生命周期、审查状态、报告、Telegram、Provider 或其他业务表。
- `POST /radar/signals/{signal_id}/model-analysis-draft` 是第一个显式手动模型草稿 API：默认关闭时不联网、不写审计；启用后只使用确定性 `/analysis` 已脱敏上下文，fallback/degraded/blocked/malformed 路径写 `model_call_logs`，并且不修改 P0/P1/P2、生命周期、审查状态、报告、Telegram、Provider 或确定性 `/analysis` 输出。Web 信号详情、Windows 客户端和 Telegram `/model_draft <id>` 已有显式手动入口。
- 模型输出进入任何 API/UI/报告前必须先走 `app.ai.analysis_output` 这类本地净化契约：只接受 JSON 草稿，忽略未知字段，固定建议标签集合，脱源 URL/domain，限制字段长度和列表条数；direct trading language 返回 blocked，malformed JSON 返回 degraded。该模块不直接调用模型、不修改现有 analysis API。
- 需要确认“数据库到底有什么”时，运行 `uv run python infra/scripts/database_inventory.py --json-output evidence/database-inventory.json`；它只读当前 `DATABASE_URL` 指向的数据库，输出脱敏后的 dialect/driver、Alembic repo head、已应用迁移、应用表存在情况和关键表计数，不输出数据库连接串、密码、行内容、Provider 原始数据、报告正文或 prompt。
...
- `server_deploy_check.py --check-m5-smoke` 会只读验证健康、OPS、Provider、Radar、Telegram、`/radar/signals` 和有信号时的 `/radar/signals/{id}/analysis` JSON 契约；analysis smoke 不只检查顶层字段存在，还会校验 5 个固定 `agent_assessments` 角色、状态取值和 findings/next_actions 数组形状；空信号列表只记录 warning，避免阻断新服务器首次部署。显式加 `--require-radar-signal-analysis-sample` 时，空信号列表会升级为失败，用于首用演示或生产化验收前确认 analysis 契约确实被抽样覆盖；该严格模式不自动 seed、不触发采集/扫描。`server_delivery_acceptance.py` 默认验收 localhost，也可用 `--base-url <url>` 验证非默认端口、反向代理或域名地址；它会串联部署预检、backup check-only、backup retention dry-run 和 runtime check，并保留 helper evidence JSON 的 warning 状态，汇总报告可见但仍零退出，缺失、损坏或失败 evidence 会标为 fail；生产切换前可加 `--fail-on-warning`，让 warning-only 验收仍写 `warn` 报告但返回非零退出码；生产化交付前可加 `--production-readiness` 一次启用当前非破坏性生产验收预设，覆盖 server compose contract、systemd 模板、Telegram 严格绑定、Tushare Beat checklist、模型 Provider readiness、严格 radar analysis 样本门禁、数据库只读清单、OPS evidence、no-send alert preview 和 Telegram alert env preflight，并在 JSON 报告中写入 `profile: "production_readiness"`；首次执行前可加 `--plan-only` 只写命令/evidence 计划报告，报告为 `execution_mode: "plan"` 和 `status: "planned"`，不调用 helper、Docker、API、Telegram、systemd 或 journalctl；该预设不启用手动 alert service verification、严格 env 权限、`--fail-on-warning`、`--fail-fast`、restore、Telegram send、backup deletion 或 systemd/journalctl 调用。需要单独控制时仍可使用 `--require-radar-signal-analysis-sample`、`--include-database-inventory`、`--include-server-compose-contract-check`、`--include-systemd-unit-check`、`--include-tushare-anns-d-beat-enablement`、`--include-model-provider-readiness`、`--include-ops-evidence`、`--include-alert-telegram-preview`、`--include-alert-telegram-env-check`、`--include-alert-telegram-service-verify`、`--skip-backup-retention` 和 `--restore-check-input backups/<file>.sql` 等开关。
...
- Telegram `/model_draft <id>` 显式手动调用 `/radar/signals/{signal_id}/model-analysis-draft`，只展示后端净化后的模型草稿状态、摘要字段、blocked count 和 boundary，不自动调用模型草稿、不展示 prompt、raw response、raw source、blocked phrase text、个人持仓成本/仓位或交易指令。
...
- Windows 客户端默认仍是 MVP 源码运行版；首次试运行推荐 `clients/windows/first-trial.ps1`，默认连接 `http://127.0.0.1:8000`、使用 `user_key=default` 和 24 小时 OPS readiness/OPS trends 窗口，并只委托 `run-client.ps1 -SmokeCheck`，通过后才打开 GUI。需要本机 Docker 后端时必须显式加 `-StartDockerBackend`；该 opt-in 路径使用 `docker-compose.yml` + `docker-compose.server.yml` 先重建当前源码的 `api` 镜像，确保 `/ops/trends` 等新端点存在，再启动 `postgres` / `redis`、通过 `api` 容器执行 `alembic upgrade head`、启动 `api` / `worker` / `beat`，并等待 `<ServerUrl>/health` 后才进入 smoke/GUI 委托，Docker build、启动、迁移失败或超时会在 GUI 前阻断；加 `-SmokeOnly` 时仍先完成同一 Docker bootstrap/health wait，然后只跑 smoke check 并退出；加 `-SeedDemoDataJsonOutput <path>` 可在 Docker health 后、database inventory/deploy preflight/smoke/GUI 前显式写入 synthetic demo 数据并保存 evidence；加 `-DatabaseInventoryJsonOutput <path>` 可在 Docker health 后、deploy preflight/smoke/GUI 前保存脱敏数据库只读清单 JSON；加 `-DeployCheckJsonOutput <path>` 可在 Docker health 后、smoke/GUI 前保存部署预检 JSON，`-DeployCheckServerComposeContract`、`-DeployCheckM5Smoke`、`-DeployCheckRequireRadarAnalysisSample` 和 `-DeployCheckBackupJsonOutput <path>` 可分别把 server compose runtime contract、只读 M5 契约、严格 radar analysis 样本门禁和 PostgreSQL backup check-only evidence 纳入同一次部署预检；Windows launcher 只追加后端脚本参数，不在 PowerShell 中解析 compose 或数据库内容。`clients/windows/package-client.ps1` 提供可选 PyInstaller onedir 打包脚手架，`clients/windows/package-installer.ps1` 可用 Inno Setup 把 onedir 包封装成未签名 preview `Setup.exe`；CI 会同时上传 onedir artifact 和 installer artifact。打包工具通过 `uv sync --group package` 和 `uv run --group package powershell ... clients/windows/package-client.ps1 ...` 使用，PyInstaller 不进入默认运行时或普通 dev 路径。`-DryRun` 可在不生成构建产物的情况下预览命令，`-CheckOnly` 可运行 Python/Tkinter/入口模块 preflight 和 PyInstaller 可用性检查后退出且不生成产物，必要时加 `-CheckJsonOutput` 保存只含打包前置条件、命令和路径元数据的有界 JSON 证据。非 dry-run 默认先验证 Python 3.12、`tkinter` import 和入口模块解析；只有本地排查特殊问题时才加 `-SkipPreflight`，且不能和 `-CheckOnly` 同用。客户端只消费后端 API，不重新计算雷达等级、市场情绪、运行状态、数据源状态或评分。首次使用也可直接用 `run-client.ps1 -SmokeCheck` 在打开 GUI 前执行只读 preflight，或用 `run-client.ps1 -SmokeOnly` 对已运行 API 只执行 preflight 并退出、不打开 GUI；必要时加 `-SmokeLookbackHours` 调整 `/ops/readiness` 和可选 `/ops/trends?lookback_hours=<selected>&bucket_count=12` 的 1 到 168 小时统计窗口、加 `-SmokeCompactJsonOutput` 保存不含 endpoint payload/raw response 的 compact evidence、加 `-SmokeJsonOutput` 保存详细脱敏 JSON，或加 `-SmokeStrict` 让 warning 阻断启动。已打开 GUI 后，可用 `OPS 趋势` 按钮只读读取 `/ops/trends?lookback_hours=<selected>&bucket_count=12`，可用 `告警钻取` 按钮只读聚合 OPS readiness/overview/history 中的后端告警字段，也可用 `首用诊断` 按钮复用同一套 smoke check，传入当前 Server URL、User Key 和 OPS Lookback，默认不写 evidence 文件、不打开第二个窗口。
- 不接自动交易，不输出强买卖指令，不保存券商交易密码。
- 新表必须有 Alembic 迁移，新规则必须有测试或 golden case。
- 后续 AI 协作默认策略：模块设计或开发阶段完成后，AI 自动做 git commit 并推送到已配置的远端，不再每次向用户确认。
