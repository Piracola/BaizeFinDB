...
- 模型输出进入任何 API/UI/报告前必须先走 `app.ai.analysis_output` 这类本地净化契约：只接受 JSON 草稿，忽略未知字段，固定建议标签集合，脱源 URL/domain，限制字段长度和列表条数；direct trading language 返回 blocked，malformed JSON 返回 degraded。当前该模块不调用模型、不落库、不改变现有 analysis API。
