...
- `POST /radar/signals/{signal_id}/model-analysis-draft` 是第一个显式手动模型草稿 API：默认关闭时不联网、不写审计；启用后只使用确定性 `/analysis` 已脱敏上下文，fallback/degraded/blocked/malformed 路径写 `model_call_logs`，并且不修改 P0/P1/P2、生命周期、审查状态、报告、Telegram、Provider 或确定性 `/analysis` 输出。Web 信号详情已有手动 `生成模型草稿` 按钮；Windows 和 Telegram 仍未接入。
