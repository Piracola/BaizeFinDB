...
- `/ops/overview`、`/ops/history` 和 `/ops/readiness` 是当前只读运行状态、服务端磁盘/CPU/内存摘要、运维历史、运行就绪自检和告警摘要入口，可用于本地排障、Windows GUI 告警钻取、服务器 smoke check、`server_runtime_check.py` 运行采样和后续监控接入。
