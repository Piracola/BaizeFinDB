...
`-DeployCheckM5Smoke`；如果已经通过 demo seed 或真实扫描准备了信号样本，并希望
`/radar/signals/{id}/analysis` 必须被抽样验证，再加
`-DeployCheckRequireRadarAnalysisSample`。需要同时保存 PostgreSQL backup check-only
evidence 时，加 `-DeployCheckBackupJsonOutput <path>`：
