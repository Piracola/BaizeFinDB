...
这些接口只聚合已有记录和 API 进程运行信息，不会触发采集、扫描、推送或模型调用。磁盘空间检查默认读取当前工作目录，可通过 `OPS_DISK_CHECK_PATH` 和 `OPS_DISK_FREE_PERCENT_ALERT_THRESHOLD` 调整；运维历史重点看 `recent_events` 和 `failure_summary`。
