...
| `infra/scripts/server_deploy_check.py` | 服务器部署预检脚本，验证 `.env`、compose 配置、可选镜像构建、容器状态、API 健康检查、Ops 运行状态、AKShare/Tushare 状态和 M5 只读 smoke check。 |
...
验证 M5 核心只读接口 JSON 契约，包含 `/ops/overview` 的运行状态、服务端磁盘摘要、AKShare 状态和 Tushare 状态：
