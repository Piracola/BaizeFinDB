...
需要只把数据库清单加入默认验收包时，可用
`--include-database-inventory`；它只新增 `database_inventory` stage 并写出
`database-inventory.json`，不执行迁移、seed、备份、恢复、采集、扫描、报告、推送或模型调用。
...
定时巡检或后续告警发送器更适合消费 compact summary，而不是完整 runtime
report。`server_monitor_check.py` 复用同一套只读 runtime check，默认单次采样且
不等待，输出 `ok` / `warning` / `blocked`，`blocked` 返回非零；需要 warning
也阻断定时任务时加 `--fail-on-warning`。它不发通知、不写数据库、不触发采集、
扫描、推送、模型、报告、备份或清理：
...
`baizefindb-alert-telegram.service` 会在真正发送前以 `baizefindb` 服务账号自动运行同一
env 预检，并写 `evidence/server-alert-telegram-env-check.json`。如果该文件是
`root:root` 且 `0600`，服务账号无法读取，service 会在发送前失败；把 owner 调整为
实际运行 service 的账号后再重试。
...
`evidence/server-alert-telegram-dedupe-state.json`，并在发送前写
...
timer 会写出 `evidence/server-monitor-summary.json`、
`evidence/server-runtime-monitor.json` 和
`evidence/server-alert-payload.json`。alert payload 只是后续发送 adapter 可消费的
no-send JSON，不发送通知。若你希望 warning-only 巡检也让
