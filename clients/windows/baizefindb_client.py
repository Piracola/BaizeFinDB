...
class BaizeFinDBClientApp:
    ...
    def _build_ui(self) -> None:
        ...
        button_frame.grid(row=1, column=0, columnspan=2, sticky=tk.EW, pady=(10, 10))
        ...
        self.output.grid(row=2, column=0, columnspan=2, sticky=tk.NSEW)
        self.output.insert(
            tk.END,
            "BaizeFinDB Windows 客户端 MVP\n\n"
            "输入 API 地址后选择操作。雷达等级、生命周期和审查状态均来自后端 API。\n",
        )
        self.output.configure(state=tk.DISABLED)

        status_bar = ttk.Label(main, textvariable=self.status_text, anchor=tk.W)
        status_bar.grid(row=3, column=0, columnspan=2, sticky=tk.EW, pady=(8, 0))
