...
这是 Windows 客户端 MVP：用 Python 标准库和 Tkinter 连接 BaizeFinDB API，查看健康状态、雷达总览、信号列表、持仓、自选、报告摘要、日报/周报汇总、单信号综合评分，维护 Telegram chat 绑定/白名单，并打开现有 Web 面板。
...
- 日报/周报和综合评分也来自后端；客户端只负责触发、读取和展示。
