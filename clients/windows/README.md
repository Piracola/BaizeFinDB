...
- `first-trial.ps1 -StartDockerBackend -DeployCheckJsonOutput <path>` 会在 Docker 后端健康后运行 `infra/scripts/server_deploy_check.py --check-containers --check-api --json-output <path>`，失败时不进入 smoke/GUI。
