...
| 阶段 | 目标 | 完成标准 |
| --- | --- | --- |
| M1 工程骨架 | 搭出可启动的最小服务端 | FastAPI、PostgreSQL、Redis、Alembic、Docker Compose、`GET /health`、pytest 冒烟测试可用。 |
| M2 数据底座 | AKShare Provider 跑通 | 行情/板块/概念数据可拉取、入库、记录质量标签；失败不拖垮主服务。 |
| M3 雷达核心 | 生成 P0/P1/P2 信号和生命周期 | 每次扫描可生成候选信号，能追溯数据批次、规则和证据。 |
