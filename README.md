...
Linux 服务器不能用 PyInstaller 直接交叉生成 Windows `.exe`。仓库提供 `.github/workflows/windows-client-package.yml`，推送到 GitHub 后会在 `windows-latest` runner 上运行同一份 `clients/windows/package-client.ps1`，并用 `BAIZEFINDB_PACKAGED_IMPORT_CHECK=1` 启动打包后的 exe 做导入校验，再产出 `BaizeFinDB-Windows-Client-<commit>` artifact；该 artifact 是 onedir Windows 客户端包，不提交到仓库。
