...
- `/settings/status` 提供统一只读设置状态摘要：汇总 Tushare token/`anns_d` Beat、Telegram bot/白名单/webhook/推送开关和模型 provider readiness。该接口不读数据库、不调用外部 API、不验证 token、不写 `.env`、不发送 Telegram、不触发扫描/报告/模型调用，返回值只包含布尔、计数、状态和脱敏检查说明。未来若要做“浏览器填写并保存 API key”的设置页，必须先补本地管理员认证或 localhost-only 写入边界。
