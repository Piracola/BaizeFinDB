...
- `/settings/status` 提供统一只读设置状态摘要：汇总 Tushare token/`anns_d` Beat、Telegram bot/白名单/webhook/推送开关和模型 provider readiness。该接口不读数据库、不调用外部 API、不验证 token、不写 `.env`、不发送 Telegram、不触发扫描/报告/模型调用，返回值只包含布尔、计数、状态和脱敏检查说明。`GET /settings/editable` 返回可编辑字段元数据和脱敏配置状态，`PUT /settings/editable` 可在 owner-only/localhost 边界下写入服务器本地 `.env` 并刷新当前 API 进程配置缓存，secret 字段默认留空不修改、勾选清空才会移除值；worker/beat/Docker 进程可能仍需重启才能读取新环境。`POST /settings/connection-test` 是显式手动连接测试，支持 `server`、`tushare`、`telegram`、`model`、`all`，会调用选定外部 provider 但不写数据库、不发送 Telegram 消息、不改雷达/报告，结果只返回脱敏状态。
...
- Telegram Bot MVP Webhook 模块：只消费健康检查、运行状态、运维历史、OPS 趋势桶、运行就绪自检、OPS 告警钻取、Tushare 数据源状态和准入自检、雷达、报告、单信号后端分析摘要、手动模型草稿和评分后端结果，`/ops` 展示后端运行状态摘要，`/ops_history` 展示只读运维异常历史，`/ops_trends` 固定读取 24 小时 / 12 桶后端趋势计数，展示扫描、失败和 unhealthy 桶摘要，不重算 OPS readiness 或状态，`/ops_ready` 展示运行就绪自检，`/ops_warn` 展示只读 OPS 告警钻取，`/tushare` 展示 Tushare 只读配置状态，`/tushare_ready` 展示 Tushare 抓取/调度准入自检，`/radar` 展示后端市场情绪摘要，`/analysis <id>` 展示后端单信号研究摘要和确定性 agent assessments，`/model_draft <id>` 只在显式命令下调用后端模型草稿接口并展示净化后的状态、摘要、blocked count 和 boundary，不重新计算 P0/P1/P2、OPS 状态、分析摘要、agent 状态、模型状态或评分
- `/telegram/status` 查看 Telegram 配置状态，不泄露 token 或 secret
- `/telegram/webhook` 接收 Telegram update，支持 `/help`、`/id`、`/health`、`/ops`、`/ops_history`、`/ops_trends`、`/ops_ready`、`/ops_warn`、`/tushare`、`/tushare_ready`、`/radar`、`/signals`、`/signal <id>`、`/analysis <id>`、`/model_draft <id>`、`/holding`、`/watchlist`、`/reports`、`/daily`、`/weekly`、`/score <id>`；`/health` 展示最近扫描状态，`/ops` 展示运行状态摘要，`/ops_history` 展示只读运维异常历史，`/ops_trends` 用 Telegram 24 小时窗口和 12 个桶只读展示后端 OPS 趋势桶计数，不从桶计数推导 readiness 或运行状态，`/ops_ready` 展示运行就绪自检，`/ops_warn` 用 Telegram 24 小时窗口只读复用 `/ops/readiness`、`/ops/overview` 和 `/ops/history`，优先展示后端 readiness、非 OK 检查、alerts、failure_summary 和有界 recent events，`/tushare` 只读展示 Tushare token 配置和端点实现状态，`/tushare_ready` 展示 Tushare token、最新抓取和数据质量准入状态，`/analysis` 展示后端单信号研究摘要，`/model_draft` 显式手动展示后端净化后的模型草稿，`/score` 展示后端 v2 评分档位和组件明细
...
Windows 客户端位于 [clients/windows/](clients/windows/)，默认是源码运行版。仓库提供可选 PyInstaller onedir 打包脚手架，也提供基于 Inno Setup 的普通 `Setup.exe` 安装包脚手架；当前安装包仍是未签名 preview，不包含自动更新或生产分发承诺。
...
可选安装包脚手架会把上一步生成的 onedir 包封装为普通 Windows `Setup.exe`，需要先安装 Inno Setup 6：
...
- `/model_draft <id>` 显式手动调用 `POST /radar/signals/{signal_id}/model-analysis-draft`，展示后端净化后的 model/draft 状态、模型元数据、advisory 字段、blocked count 和 boundary；Telegram 不自动调用模型草稿、不展示 prompt、raw response、raw source、blocked phrase text、个人持仓成本/仓位或交易指令。
